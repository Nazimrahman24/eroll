...
def extract_from_cell_text(text):
    # Pattern 1: 3 letters + 7 digits (existing)
    pattern1 = r"[A-Z]{3}\s*\d{7}"
    # Pattern 2: New format like BR/19/108/693144
    pattern2 = r"[A-Z]{2}/\d{2}/\d{3}/\d{6}"

    voter_id_match = re.search(pattern1, text)
    if not voter_id_match:
        voter_id_match = re.search(pattern2, text)
