...
def extract_from_cell_text(text):
    pattern1 = r"[A-Z]{3}\s*\d{7}"
    pattern2 = r"[A-Z]{2}/\d{2}/\d{3}/\d{6}"

    voter_id_match = re.search(pattern1, text) or re.search(pattern2, text)
...
# Process PDF with parallelization
def process_pdf_with_google(pdf_path, save_path, max_workers=4):
    all_voters = []
    info = pdfinfo_from_path(pdf_path)
    total_pages = info['Pages']

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_page, pdf_path, page): page for page in range(1, total_pages + 1)}
        for future in as_completed(futures):
            all_voters.extend(future.result())
